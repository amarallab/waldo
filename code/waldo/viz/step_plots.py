...
class StepPlot2(object):
    ...
    def step_facets(self, df, df2, t1=10, t2=120, rescale_to_hours=True):
        xmax = max([max(df['tN']), max(df['tN'])])
        label_size = 16
        tick_label_size = 12
        gs = grd.GridSpec(3, 1)
        ...
        ax_l0 = plot.add_subplot(gs[2, 0], axisbg='white')
        #plt.yticks([0, 10, 100], [0, 50, 100], fontsize=tick_label_size)
        plt.yticks([0, 10, 20], [], fontsize=tick_label_size)
        plt.xticks([0, 4, 8, 12], [], fontsize=tick_label_size)
        ax_l0.set_ylim([0, 20])
        ax_m0 = plot.add_subplot(gs[1, 0], axisbg='white')
        plt.yticks([0, 100], [0, 100], fontsize=tick_label_size)
        plt.yticks([0, 100], [], fontsize=tick_label_size)
        plt.xticks([0, 4, 8, 12], [], fontsize=tick_label_size)
        ax_s0 = plot.add_subplot(gs[0, 0], axisbg='white')
        plt.yticks([0, 100], [0, 100], fontsize=tick_label_size)
        plt.yticks([0, 100], [], fontsize=tick_label_size)
        plt.xticks([0, 4, 8, 12], [0, 4, 8, 12], fontsize=tick_label_size)
        plt.xticks([0, 4, 8, 12], [], fontsize=tick_label_size)
        # ax_l1 = plot.add_subplot(gs[0, 1], sharey=ax_l0, axisbg='white')
        # ax_m1 = plot.add_subplot(gs[1, 1], sharey=ax_m0, axisbg='white')
        # ax_s1 = plot.add_subplot(gs[2, 1], sharey=ax_s0, axisbg='white')
        ...
        self._new_plot(ax_s0, short1, 'raw', xmax=xmax, alpha=0.7)
        self._new_plot(ax_m0, mid1, 'raw', xmax=xmax, alpha=0.7)
        self._new_plot(ax_l0, long1, 'raw', xmax=xmax, alpha=0.7)
