...
def smooth_and_space(times, x, y, smooth=(25, 5), dt=1.0, ndigits=0):
    """
...
def xy_to_full_dataframe(times, x, y):
    ...
    dt = np.diff(np.array(times)).sum() / len(times)
    dt= round(dt, ndigits=1)
    mdata = markov_measures_for_xy(x, y, dt=dt)
    r, s, a, ar = zip(*mdata)
    #print type(r[0])
    #print type(s[0])
    #print type(a[0])
    ...
    #print len(r), len(s), len(a), len(ar)
    data = zip(x[1:-1], y[1:-1], speeds[1:-1], angles[1:-1], r, s, a, ar)
    #print len(data)
    #print data[:2]
    df = pd.DataFrame(data,
                      index=times[1:-1],
                      columns=['x', 'y', 'v', 'dtheta', 'r', 'v2', 'a', 'ar'])
