...
def create_entries_from_blobs_files(ex_id, min_body_lengths, min_duration, min_size, 
    ...
    if store_tmp:
        for local_id, blob in raw_blobs.iteritems():
            ...
            write_tmp_file(data=metadata, blob_id=blob_id, data_type='metadata')
            data = {'time':blob['time'], 'data':blob['xy']}
            write_tmp_file(data=data, blob_id=blob_id, data_type='xy_raw')
            data = {'time':blob['time'], 'data':blob['outline']}
            write_tmp_file(data=data, blob_id=blob_id, data_type='encoded_outline')            
            data = {'time':blob['time'], 'data':blob['aspect_ratio']}
            write_tmp_file(data=data, blob_id=blob_id, data_type='aspect_ratio')            
    if store_db:
        write_raw_blobs_to_db(raw_blobs, metadata_docs, **kwargs)
