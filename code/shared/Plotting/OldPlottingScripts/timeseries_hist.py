...
#from scatterplot_2d_preprocesing import *
...
if __name__ == '__main__':
    ...
    query = {'data_type':'speed_filtered'}
    projection = {'blob_id':1, 'part':1}
    blobs_with_data = mongo_query(query, projection)
    
    unique_blobs = list(set([entry['blob_id'] for entry in blobs_with_data]))
    print len(unique_blobs), 'blobs found with data'
    for blob_id in unique_blobs[:1]:
        print blob_id
        #calculate_widths_for_blob_id(blob_id)
