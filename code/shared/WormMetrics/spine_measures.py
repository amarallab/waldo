...
def compute_length(spine, scaling_factor=1):
    tot_distance = 0.
...
def compute_all_lengths(times, spine_list, scaling_factor=1):
    length_timedict = {}

    for i in range(len(times)):
        if len(spine_list[i]) > 0:
            length_timedict[make_it_str(times[i])] = compute_length(spine_list[i], scaling_factor)
        else:
            length_timedict[make_it_str(times[i])] = 'skipped'
...
def compute_spine_measures(blob_id, metric='all', smooth=False, source_data_entry=None, **kwargs):
    ...
    length_timedict = compute_all_lengths(times, spine_list)
    median_length = sorted(length_timedict.values())[len(length_timedict) / 2]
