...
def silent_remove(filename):
    """ delete a file. if file does not exist, no error is raised. """
    try:
        os.remove(filename)
    except OSError:
        pass
...
def get_dset(ex_id):
    """ returns the dataset that a ex_id belongs to. default is: 'something' """
...
def format_directory(ID_type, dataset='', tag='', ID='',
    ...
    worm_dir: (str)
        the directory in which all worm level data should be saved
    plate_dir: (str)
        the directory in which all plate level data should be saved
    dset_dir: (str)
        the directory in which all dataset level data should be saved    
    """
...
def format_filename(ID, ID_type='worm', data_type='cent_speed', 
                    ...
                    worm_dir=WORM_DIR, plate_dir=PLATE_DIR, dset_dir=DSET_DIR):
    """
    returns the name of a file which matches the specified parameters.
    automatically creates the directory for that file if it does not exist.

    the general form for worms is:
        {dir}/{ex_id}/{blob_id}-{data_type}.{file_type}

    the general form for plates and datasets is:
        {dir}/{dataset}/{tag}/{id}-{data_type}.{file_type}
    ...
    file_dir: (str or None)
        the path to a non-default directory you want to save the file in.
    worm_dir: (str)
        the default directory in which all worm level data should be saved
    plate_dir: (str)
        the default directory in which all plate level data should be saved
    dset_dir: (str)
        the default directory in which all dataset level data should be saved
...
def get_timeseries(ID, data_type, 
    ...
    """
    reads a h5 file containing a pandas dataframe and returns the dataframe.
    used for individual worms but not plates or datasets.
...
def write_timeseries_file(ID, data_type, times, data, 
                          ...
                          file_dir=None, **kwargs):
    """
    writes a file containing timeseries data. currently only used for
    individual worm timeseries.
    ...
    returns:
    success: (bool)
       returns true if data exists and file was written.
    """    
...
def write_table(ID, data_type, dataframe, ID_type='p',                
                dset=None, file_tag='', file_dir=None, **kwargs):                
    """
    writes an h5 file containing a pandas dataframe.
    used for plate and dataset level aggregation, but not for individual worms.
    
    the general form for savenames is:
        {dir}/{dataset}/{tag}/{id}-{data_type}.h5
